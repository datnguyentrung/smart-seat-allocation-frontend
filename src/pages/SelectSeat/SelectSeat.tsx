import selectSeatStyle from "./SelectSeat.module.scss";
...
export default function SelectSeat() {
  ...
  const wsStatusClass =
    wsStatus === "connected"
      ? selectSeatStyle.selectSeatWsConnected
      : wsStatus === "connecting"
        ? selectSeatStyle.selectSeatWsConnecting
        : wsStatus === "error"
          ? selectSeatStyle.selectSeatWsError
          : selectSeatStyle.selectSeatWsDisconnected;
  ...
  if (isLoading || !showtimeDetails || seats.length === 0) {
    return (
      <div
        className={selectSeatStyle.selectSeatApp}
        style={{
          display: "flex",
  ...
  return (
    ...
    >
      ...
      {ENABLE_WEBSOCKET && (
        <div className={selectSeatStyle.selectSeatWsStatus}>
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            className={`${selectSeatStyle.selectSeatWsIndicator} ${wsStatusClass}`}
          ...
          >
            {wsConnected ? (
              ...
              <>
                <RefreshCw
                  size={16}
                  className={selectSeatStyle.selectSeatSpinning}
                />
              ...
              <>
                ...
                <button
                  onClick={wsReconnect}
                  className={selectSeatStyle.selectSeatReconnectBtn}
                  title="Reconnect"
                >
                  ↻
                </button>
      ...
      <main className={selectSeatStyle.selectSeatMain}>
        {/* Flex Container for BookingControls and Screen+Seats */}
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.1 }}
          className={selectSeatStyle.selectSeatContainer}
        >
          {/* BookingControls - Left Side */}
          <div className={selectSeatStyle.selectSeatControlsSection}>
            <BookingControls
              onReset={handleReset}
        ...
        <motion.div
          ...
          className={selectSeatStyle.selectSeatLegendWrapper}
        >
          <Legend />
        </motion.div>

        <div className={selectSeatStyle.selectSeatTerms}>
          <p>
      ...
      {isBookingSummaryOpen && (
        <div className={selectSeatStyle.selectSeatModalOverlay}>
          <div className={selectSeatStyle.selectSeatModalContent}>
            <BookingSummary
              movieTitle="Avatars: The Way of Water"
              selectedSeats={selectedLabels}
      ...
      {isBookingProcessing && (
        <div className={selectSeatStyle.selectSeatModalOverlay}>
          <motion.div
            ...
            className={selectSeatStyle.selectSeatLoadingContainer}
          >
            <motion.div
              className={selectSeatStyle.selectSeatSpinner}
              animate={{ rotate: 360 }}
              transition={{
                duration: 1,
                repeat: Infinity,
            ...
            />
            <p className={selectSeatStyle.selectSeatLoadingText}>
              Đang xử lý đặt vé...
            </p>
          </motion.div>
        </div>
